...
export async function install_plugins(h5wasm_module, names=plugin_names, new_plugin_path=null) {
    if (new_plugin_path !== null) {
        h5wasm_module.insert_plugin_search_path(new_plugin_path, 0);
    }
    const plugin_path = h5wasm_module.get_plugin_search_paths()[0];
    h5wasm_module.FS.mkdirTree(plugin_path);
    ...
    for (const plugin_name of names) {
        ...
        h5wasm_module.FS.writeFile(`${plugin_path}/${plugin_filename}`, new Uint8Array(buf));
    }
}

export function list_plugins(h5wasm_module) {
    const plugin_path = h5wasm_module.get_plugin_search_paths()[0];
    return h5wasm_module.FS.readdir(plugin_path);
}

export function install_local_plugins(h5wasm_module) {
    const local_path = base_url.replace(/^file:\/\//, '');
    h5wasm_module.insert_plugin_search_path(`${local_path}/plugins`, 0);
}
